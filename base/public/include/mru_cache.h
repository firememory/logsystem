...
class MRUCacheBase {
 ...
 public:
  ...
  // 
  iterator GetLowerBound(const KeyType& key) {
    typename KeyIndex::iterator index_iter = index_.lower_bound(key);
    if (index_iter == index_.end())
      return end();
    if (index_iter->first > key) { --index_iter; }
    if (index_iter == index_.end())  return end();
    typename PayloadList::iterator iter = index_iter->second;
...
//     return ordering_.begin();
  ...
  iterator GetUpperBound(const KeyType& key) {
    typename KeyIndex::iterator index_iter = index_.upper_bound(key);
    if (index_iter == index_.end())
      return end();    
    typename PayloadList::iterator iter = index_iter->second;
...
class MRUCacheNullDeletor {
 public:
  void operator()(PayloadType& payload) { UNUSED_PARAM(payload);
  }
};

// A container that does not do anything to free its data. Use this when storing
// value types (as opposed to pointers) in the list.
template <class KeyType, class PayloadType, class DeletorType = MRUCacheNullDeletor<PayloadType>>
