...
#define _DEPRECATED CHROME_OLD__DEPRECATED
...
struct char_t_hash {
  ...
  std::size_t operator()(const char_t* s) const {
    ASSERT(s);
    std::size_t result = 0;
  ...
  bool operator()(const char_t* s1,const char_t* s2) const {
    ASSERT(s1 && s2);
    return 0 == STRCMP(s1, s2) ? false : true;
  }
};
